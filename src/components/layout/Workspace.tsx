...
export const Workspace: React.FC<{ sidebarWidth: number }> = ({ sidebarWidth }) => {
  ...
  const renderContent = () => {
    ...
    if (result) {
      ...
      if (Comp) {
        return <Comp tableId={result.tableId} resultId={result.id} />;
      }
      return <div className="p-4 text-gray-500">未知分析类型: {result.type}</div>;
    }
        ...
        {openTabs.map((tabId) => {
          ...
          return (
            ...
            >
              ...
              <button
                className="ml-1 p-0.5 rounded hover:bg-gray-300 flex-shrink-0"
                onClick={(e) => {
                  e.stopPropagation();
                  closeTab(tabId);
                }}
