...
export const NormalityTest: React.FC<NormalityTestProps> = ({ tableId, resultId }) => {
  ...
  const chartOption: EChartsOption | null = useMemo(() => {
    ...
    for (let i = 0; i <= bins; i++) binEdges.push(chartMin + i * binWidth);
    ...
    for (let x = chartMin; x <= chartMax; x += curveStep) {
      ...
      curveY.push((n * binWidth / (overallStddev * Math.sqrt(2 * Math.PI))) * Math.exp(-0.5 * z * z));
    }
    const markLines: Record<string, unknown>[] = [];
    if (isFinite(specLsl)) {
      markLines.push({ name: 'LSL', xAxis: specLsl, lineStyle: { color: '#ef4444', type: 'dashed' as const, width: 2 }, label: { formatter: `LSL=${specLsl.toFixed(3)}`, position: 'start' as const } });
    }
    if (isFinite(specUsl)) {
      markLines.push({ name: 'USL', xAxis: specUsl, lineStyle: { color: '#ef4444', type: 'dashed' as const, width: 2 }, label: { formatter: `USL=${specUsl.toFixed(3)}`, position: 'end' as const } });
    }
    if (isFinite(specTarget)) {
      markLines.push({ name: '目标', xAxis: specTarget, lineStyle: { color: '#10b981', type: 'dashed' as const, width: 2 }, label: { formatter: `目标=${specTarget.toFixed(3)}` } });
    }
    markLines.push({ name: '均值', xAxis: mean, lineStyle: { color: '#3b82f6', type: 'solid' as const, width: 1.5 }, label: { formatter: `X\u0304=${mean.toFixed(3)}` } });
    return {
      ...
      series: [
        { type: 'bar', data: histData.map((v, i) => [chartMin + (i + 0.5) * binWidth, v]), name: '频次', color: '#3b82f6', barWidth: '99%', barCategoryGap: '0%', barGap: '0%' },
        { type: 'line', data: curveX.map((x, i) => [x, curveY[i]]), name: '正态拟合', smooth: true, color: '#ef4444', lineStyle: { width: 2 }, symbol: 'none' as const, markLine: { silent: true, symbol: 'none' as const, data: markLines } },
      ],
    };
          ...
          <div className="flex gap-3">
            <div className="w-48 flex-shrink-0 text-xs">
              <table className="w-full border-collapse">
                <tbody>
                  {[
                    ['样本量', resultData.n],['均值', resultData.mean.toFixed(4)],['规格下限', resultData.lsl.toFixed(4)],['目标中值', resultData.target.toFixed(4)],['规格上限', resultData.usl.toFixed(4)],['整体标准差', resultData.overallStddev.toFixed(4)],['组内标准差', resultData.withinStddev.toFixed(4)],['组间标准差', resultData.betweenStddev.toFixed(4)],
                  ].map(([l, v]) => (<tr key={l as string} className="border-b border-gray-100"><td className="py-0.5 text-gray-500">{l}</td><td className="py-0.5 text-right font-mono text-gray-700">{typeof v === 'number' ? v : v}</td></tr>))}
                </tbody>
              </table>
            </div>
            <div className="flex-1 min-w-0">{chartOption && <ChartContainer option={chartOption} height={380} />}</div>
            ...
            {normResult && (
              <div className="w-80 flex-shrink-0 text-xs">
                <table className="w-full border-collapse border border-gray-200">
                  ...
                  <tbody>
                    <tr><td className="px-2 py-1 border border-gray-200">AD</td><td className="px-2 py-1 border border-gray-200 text-right font-mono">{normResult.ad.statistic.toFixed(4)}</td><td className="px-2 py-1 border border-gray-200 text-right font-mono">{normResult.ad.pValue.toFixed(4)}</td></tr>
                    <tr><td className="px-2 py-1 border border-gray-200">SW</td><td className="px-2 py-1 border border-gray-200 text-right font-mono">{normResult.sw.statistic.toFixed(4)}</td><td className="px-2 py-1 border border-gray-200 text-right font-mono">{normResult.sw.pValue.toFixed(4)}</td></tr>
                    <tr><td className="px-2 py-1 border border-gray-200">KS</td><td className="px-2 py-1 border border-gray-200 text-right font-mono">{normResult.ks.statistic.toFixed(4)}</td><td className="px-2 py-1 border border-gray-200 text-right font-mono">{normResult.ks.pValue.toFixed(4)}</td></tr>
                  </tbody>
                </table>
              </div>
            )}
