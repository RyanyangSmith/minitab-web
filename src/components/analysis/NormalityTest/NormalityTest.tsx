...
export const NormalityTest: React.FC<NormalityTestProps> = ({ tableId, resultId }) => {
  ...
  const savedResult = results[resultId];
  const savedConfig = savedResult?.config as { col?: number } | undefined;
  const savedData = savedResult?.data as { result?: ReturnType<typeof normalityTest> } | undefined;

  const [selectedCol, setSelectedCol] = useState<number | null>(savedConfig?.col ?? null);
  const [resultData, setResultData] = useState<ReturnType<typeof normalityTest> | null>(
    savedData?.result ?? null
  );
  const [hasRun, setHasRun] = useState(savedData?.result != null);
  ...
  const handleRun = () => {
    ...
    setHasRun(true);
    updateResultData(resultId, { col: selectedCol }, { result } as Record<string, unknown>);
  };
  ...
  const qqOption: EChartsOption | null = useMemo(() => {
    ...
    return {
      ...
      series: [
        { type: 'scatter', data: theoretical.map((t, i) => [t, observed[i]]), name: '数据点', symbolSize: 4 },
        { type: 'line', data: [[Math.min(...observed), Math.min(...observed)], [Math.max(...observed), Math.max(...observed)]], lineStyle: { color: '#ef4444', type: 'dashed', width: 1 }, symbol: 'none' },
      ],
    };
  }, [resultData, selectedCol, tableId, getColumnData]);
    ...
    <div className="p-4 space-y-4">
      <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 space-y-3">
        ...
        <div className="flex items-center gap-2">
          <label className="text-sm font-medium text-gray-600 whitespace-nowrap min-w-[80px]">选择数据列</label>
          <select className="flex-1 px-2 py-1.5 border border-gray-300 rounded text-sm bg-white focus:border-blue-400 outline-none" value={selectedCol == null ? '' : selectedCol} onChange={(e) => setSelectedCol(e.target.value === '' ? null : Number(e.target.value))}>
            <option value="">请选择列...</option>
            {options.map((opt) => (<option key={opt.value} value={opt.value}>{opt.label}</option>))}
          </select>
          <button onClick={handleRun} disabled={selectedCol == null} className="px-4 py-2 bg-blue-600 text-white text-sm rounded hover:bg-blue-700 disabled:opacity-40 disabled:cursor-not-allowed">
            执行分析
          </button>
        </div>
      </div>
      {resultData && (
        <div className="space-y-4">
          <StatisticalTable title="正态检验结果" columns={[
            { key: 'test', label: '检验方法' }, { key: 'statistic', label: '统计量' }, { key: 'pValue', label: 'P 值' }
          ]} data={[
          ...
          <StatisticalTable title="描述统计" columns={[
            { key: 'label', label: '统计量' }, { key: 'value', label: '值' }
          ]} data={[
            { label: '样本量 (N)', value: resultData.n }, { label: '均值', value: resultData.mean }, { label: '标准差', value: resultData.stddev },
          ]} />
