...
export const ScatterPlot: React.FC<ScatterPlotProps> = ({ tableId, resultId }) => {
  ...
  const chartOption: EChartsOption | null = useMemo(() => {
    ...
    for (let r = 2; r < table.rows; r++) {
      ...
      if (xv != null && yv != null && xv !== '' && yv !== '') {
        ...
        if (!isNaN(xn) && !isNaN(yn)) {
          let group = '';
          if (groupCol != null) { const gv = getCellValue(tableId, r, groupCol); group = gv == null ? '' : String(gv); }
    ...
    return {
      ...
      xAxis: { type: 'value' as const, name: columnName(xCol) },
      yAxis: { type: 'value' as const, name: columnName(yCol) },
      series,
    };
